...
def eval_b1(cfg):
    ...
    log_metrics({"test_acc": acc, "test_f1": f1})
    ...
    with open(os.path.join(cfg["output"]["results_dir"], "tables", "classification_report_test.txt"), "w") as f:
        f.write(report)

    with open(os.path.join(cfg["output"]["results_dir"], "tables", "B1_test_metrics.csv"), "w", newline="") as f:
        writer = csv.writer(f)
